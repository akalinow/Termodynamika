...
\begin{equation}
  n_a C_v T_a + n_b C_v T_b = n C_v T
\end{equation}
z czego wynika, że:
\begin{equation}
  T = f_a T_a + f_b T_b, \quad f_a = \frac{n_a}{n_a+n_b},\  f_b = \frac{n_b}{n_a + n_b}.
\end{equation}
Temperatura końcowa jest więc średnią ważoną temperatur gazów 
w poszczególnych częściach zbiornika z wagami $f_a$, $f_b$.
...
\begin{equation}
\Delta S  =S_2 - S_1 = (n_a + n_b) C_v \ln (p_a T_a + p_b T_b) + 
(n_a + n_b) R\ln (p_aY_a + p_bY_b) - n_a C_v \ln T_a  -n_b C_v \ln T_b
- n_a R \ln Y_a - n_b R \ln Y_b
\end{equation}
gdzie oznaczyliśmy $Y_{a/b}=V_{a/b}/n_{a/b}$
. Możemy więc napisać:
\begin{equation}
\Delta S   =(n_a + n_b)[ C_v (\ln (f_a T_a + f_b T_b)  - f_a\ln T_a - f_b\ln T_b) + 
           R(\ln (f_a Y_a + f_b Y_b)  - f_a\ln Y_a - f_b\ln Y_b)]
\end{equation}
Skorzystamy teraz z wklęsłości funkcji logarytm. 
Wklęsłość oznacza, że dla $f_1 \geq 0, f_2 \geq 0, f_1+f_2 =1$ mamy:
\begin{equation}
\ln(f_a x_a + f_b x_b) \geq f_a \ln(x_a) + f_b \ln (x_b).
\end{equation}
Czyli wartość funkcji od średniej argumentów jest większa niż średnia wartość funkcji.
Stosując powyższą nierówność dla $x=T,Y$ i  dostajemy:
\begin{equation}
\Delta S  \geq  0,
\end{equation}
