...
\zadanie
Cienka, ruchoma przegroda diatermiczna dzieli zbiornik na 2 części o objętościach $V_a$ i $V_b$.
...
W pewnym momencie zwolniono przegrodę pozwalając na jej przemieszczenie.
Oblicz zmianę entropii po ustaleniu się stanu równowagi i wyrównaniu się temperatury. 
Pokaż, że jest większa od zera.
...
\begin{equation}
S_1 = n_a C_v \ln T_a  + n_a R \ln (V_a/n_a)  +  
      n_b C_v \ln T_b  + n_b R \ln (V_b/n_b)  + 
      S_{a} + S_{b}
\end{equation}

Po zwolnieniu przegrody ciśnienie i temperatura w obu częściach zbiornika będą równe:
\begin{equation}
p'_a = p'_b = p, \quad T'_a = T'_b = T.
\end{equation}

Energia przepływa z jednej części do drugiej w postaci pracy i ciepła, ale 
całkowita energia wewnętrzna gazu w obu częściach się nie zmienia. Korzystamy z tego
faktu, by obliczyć końcową temperaturę:
\begin{equation}
U_{a} + U_{b} = U'_{a} + U'_{b} \quad \Rightarrow \quad  
n_a C_v T_a + n_b C_v T_b = n_a C_v T + n_b C_v T = (n_a + n_b) C_v T
\end{equation}
\begin{equation}
  T = f_a T_a + f_b T_b, \quad f_a = \frac{n_a}{n_a+n_b},\  f_b = \frac{n_b}{n_a + n_b}.
\end{equation}

Ciśnienie w obu częściach zbiornika jest równe, więc:
\begin{equation}
  pV'_a = n_a R T \quad \Rightarrow \quad \frac{V'_a}{n_a} = \frac{R T}{p}, \quad
  pV'_b = n_b R T \quad \Rightarrow \quad \frac{V'_b}{n_b} = \frac{R T}{p}
\end{equation}

Ponieważ gazy w obu częściach są identyczne możemy je traktować jako jeden gaz
o liczbie moli $n =n_a + n_b$ zajmujący objętość $V=V_a + V_b$. 
(Można też dodać stronami równania dla obu części zbiornika.)

\begin{align*}
p(V'_a + V'_b) = (n_a + n_b) R T \quad \Rightarrow \quad \frac{V'_a + V'_b}{n_a + n_b} = \frac{R T}{p} \\
\frac{V'_a + V'_b}{n_a + n_b} = \frac{n_a}{n_a + n_b} \frac{V'_a}{n_a} + \frac{n_b}{n_a + n_b} \frac{V'_b}{n_b} = f_a \frac{V'_a}{n_{a}} + f_b \frac{V'_b}{n_{b}}
\end{align*}


Entropia końcowa wynosi:
\begin{align*}
S_2 = n_a C_v \ln T  + n_a R \ln (V'_a/n_a)  + S_{a} + n_b C_v \ln T  + n_b R \ln (V'_b/n_b)  + S_{b} = \\
= (n_a + n_b) C_v \ln T + n_a R \ln \left(\frac{V'_a}{n_a}\right) + n_b R \ln \left(\frac{V'_b}{n_b}\right) + S_{a} + S_{b} = \\
|| \frac{V'_a}{n_a} = \frac{V'_b}{n_b}  = 
    \frac{V'_a + V'_b}{n_a + n_b} = 
    \frac{V_a + V_b}{n_a + n_b} = 
    f_a\frac{V_a}{n_a} + f_b\frac{V_b}{n_b}
    || \\
= (n_a + n_b) \left[C_v \ln (f_a T_a + f_b T_b) + 
                    R \ln \left( f_a\frac{V_a}{n_a} + f_b\frac{V_b}{n_b}  \right)
 \right]  + S_{a} + S_{b}
\end{align*}
...
R \ln \left( f_a\frac{V_a}{n_a} + f_b\frac{V_b}{n_b}  \right)
 \right]  + S_{a} + S_{b} - \\ 
 \left(
      n_a C_v \ln T_a  + n_a R \ln (V_a/n_a)  +  
      n_b C_v \ln T_b  + n_b R \ln (V_b/n_b)  + 
      S_{a} + S_{b} 
      \right) = \\
            (n_a + n_b)C_v \left[\ln (f_a T_a + f_b T_b) -
            f_a \ln T_a  - f_b \ln T_b \right] + \\
            (n_a + n_b)R \left[\ln \left( f_a\frac{V_a}{n_a} + f_b\frac{V_b}{n_b}  \right) -
                          f_a\ln (\frac{V_a}{n_a})  - f_b \ln (\frac{V_b}{n_b}) \right]
\end{align*}

Człony z temperaturą i objętością molową mają taką samą postać:
\begin{equation}
\ln(f_a x_a + f_b x_b) - f_a \ln(x_a) + f_b \ln (x_b).
\end{equation}
gdzie $f_1 \geq 0, f_2 \geq 0, f_1+f_2 =1$.
Mamy tu więc różnicę wartości funkcji od średniej ważonej argumentów:
$\ln(f_a x_a + f_b x_b)$ średniej ważonej wartości funkcji: $f_a \ln(x_a) + f_b \ln(x_b)$.

\begin{center}
\resizebox{0.6\linewidth}{!}{\includegraphics{wkleslosc_log.png}}
\end{center}

Dla funkcji logarytmicznej ta różnica jest dodatni - ta własność ta nazywana 
jest \emph{wklęsłością funkcji logarytmicznej}.
...
Zmiana entropii układu wynosi:
\begin{equation}
\Delta S = \Delta S_{\t{jezioro}} + \Delta S_{\t{kostka}} = -14.6 \u{J/K} + 15.3 \u{J/K} = 0.7 \u{J/K}.
\end{equation}
...
\begin{equation}
\Delta S = \Delta S_1+\Delta S_2 = m c \left( \ln T_k/T_1  + \ln T_k/T_2   \right) = m c \ln \left(\frac{T_k^2}{T_1 T_2}\right) =
...
\resizebox{\linewidth}{!}{\includegraphics{ksiezniczka_pierscien.png}}
