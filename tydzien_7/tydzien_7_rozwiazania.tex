...
\newcounter{wskazowka}\newcommand{\wskazowka}[1][]{\addtocounter{wskazowka}{1} ~\\  {\bf \emph{Wskazówka:  }} \\}
...
\end{enumerate}
Załóż, że maszyna działa między dwoma zbiornikami o temperaturach 0\degree C i 60\degree C.\\
...
\wskazowka 
Sprawność silnika jest równa stosunkowi wykonanej pracy do pobranego ciepła w całym cyklu. Skorzystać z równań dla adiabaty, żeby znaleźć relacje między temperaturami na początku i końcu przemiany adiabatycznej. \\
...
\begin{equation}
	\eta = \frac{- \Delta W}{Q_{BC}} = 1 + \frac{Q_{DA}}{Q_{BC}} = 1 + \frac{T_A - T_D}{T_C - T_B},
\end{equation}
...
Ostatecznie
\begin{equation}
	\eta = 1 - \left( \frac{V_2}{V_1} \right)^{\kappa - 1}.
\end{equation}
...
~
\zaddom
Cykl termodynamiczny $n$ moli gazu doskonałego składa się z dwóch izoterm o temperaturach $T_1$ i $T_2$ (przy czym $T_1 > T_2$) i z dwóch izobar odpowiadających ciśnieniom $p_1$ i $p_2$ ($p_1 > p_2$).
 Oblicz sprawność silnika działającego w oparciu o taki cykl.\\[1mm]

 {\it Odpowiedź:} $\eta = \frac{R(T_1 - T_2) \ln p_1/p_2}{C_p(T_1 - T_2) + R T_1 \ln p_1/p_2}$.
...
Oblicz sprawność lodówki.\\[1mm]

{\it Odpowiedź:} $\eta = \frac{3 V_2}{V_2 - V_1}$.
%~\vspace*{1cm}
% 
...
{\it Odpowiedź:} $\eta = 1 - \frac{C_V}{C_p} - \frac{R \ln V_2/V_1}{C_p(V_2/V_1 - 1)}$.

\zaddom

Skonstruowano silnik cieplny zawierający zamknięty w cylindrze 
z tłokiem n moli gazu doskonałego o cieple molowym  $C_V$.
Grzejnikiem w silniku jest gorąca cegła o masie $m$, cieple właściwym $c$ i temperaturze początkowej $T_1$. 
Chłodnicą jest duży zbiornik wodny o stałej temperaturze $T_2<T_1$. Silnik działa w następującym cyklu:
\begin{itemize}
\item[--] na początku cyklu gaz ma objętość $V_0$ i temperaturę równą temperaturze grzejnika
\item[--] w kontakcie cieplnym z grzejnikiem gaz powoli rozpręża się do objętości~$2V_0$
\item[--] przy nieruchomym tłoku, gaz ochładza się do uzyskania równowagi cieplnej z chłodnicą
\item[--] w kontakcie cieplnym z chłodnicą gaz jest sprężany izotermicznie do objętości $V_0$
\item[--] przy nieruchomym tłoku, gaz ogrzewa się od grzejnika do uzyskania z nim równowagi cieplnej
\end{itemize}

\noindent
Zakładając, że silnik jest na tyle mały że podczas pojedynczego cyklu pracy zmiana temperatury grzejnika jest pomijalna:
\begin{enumerate}
\item Narysuj cykl pracy silnika we współrzędnych $p-V$. Znajdź sprawność silnika $\eta(T)$ w zależności od temperatury grzejnika $T$. 
Wyraź (infinitezymalną) pracę wykonaną w jednym cyklu silnika przez $\eta(T)$ i infinitezymalną zmianę temperatury grzejnika.
\item Oblicz całkowitą pracę jaką może wykonać silnik aż do osiągnięcia 
przez grzejnik temperatury chłodnicy. \\[1mm]
\end{enumerate}

{\it Odpowiedź:} 
$W = \frac{mc}{(1+\alpha)^{2}} 
\left[  
    (1+\alpha)(T_{1} - T_{2}) + T_{2} \ln \frac{T_{2}}{T_{1} + \alpha (T_{1} - T_{2})}
\right], \alpha = \frac{c_{V}}{R\ln2}
$

\zaddom
Znajdź sprawność silnika, który działa w oparciu o cykl Carnota między temperaturami 
$T_2$ i $T_1$ ($T_2<T_1$) i w którym substancją roboczą jest gaz fotonowy.\\[1mm]
