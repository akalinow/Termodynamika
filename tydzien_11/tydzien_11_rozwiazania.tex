...
$f(m) = A \cdot m^2 \cdot \exp(-m/M)$, gdzie parametr $M = 5\,$g.
\begin{enumerate}[a)]
\item Wyznacz stałą $A$.
\item Gdzie leży maksimum rozkładu $f(m)$? Jaka jest średnia waga truskawki?
\item Kontrahent potrzebuje truskawek o masach zawartych pomiędzy 15\,g a 20\,g. Jaki procent truskawek możemy mu sprzedać?
...
Maksimum, z warunku $f'(m)=0$ jest dla $m=2M$.\\
...
\[
\frac{1}{2\,\cdot 5^3}\int_{15}^{20}m^2e^{-m/5}dm \simeq 0.1851
\]

Przy oszacowaniu $f(m)\Delta m$ przyjmujemy $m=(15+20)/2$, $\Delta m= 20-15=5$
\[
f(m)\Delta m = \frac{1}{2\,\cdot 5^3} (17.5)^2e^{-17.5/5} \cdot 5 \simeq 0.1850
\]

\begin{wrapfigure}[7]{r}{0.15\linewidth}\vspace{0mm}
\resizebox{\linewidth}{!}{\includegraphics{tydzien_11_rys1.png}}
\end{wrapfigure}
\zadanie
Znaleźć liczbę mikrostanów w układzie układu $N$ nierozróżnialnych kulek rozmieszczonych w pudełku z $V$ przegródkami, w każdej przegródce mieści się tylko jedna kulka ($V$ oraz $N$ definiują makrostan układu). Wykonać obliczenia dla $V = 20$ i $N = 10$ za pomocą ścisłego wzoru oraz przybliżając wynik używając wzór Stirlinga: $\displaystyle n! \approx \sqrt{2 \pi n} \left(\frac{n}{e}\right)^n$.
\vspace{5mm}
...
{\bf \em Rozwiązanie:} 
\begin{enumerate}[a)]
    \item $E_A=q_A\epsilon$, $E_B=q_B\epsilon$, $q_A=q_{A1}+q_{A2}$, $q_B=q_{B1}+q_{B2}$,  $q_{A12,B12}\geq 0$ dla całkowitych $q$. Wtedy $\Omega_A=6$, $\Omega_B=2$, czyli $\Omega_{A \cdot B}=6\cdot 2=12$.
    \item Do obliczenia $\Omega_{A+B}$ zakładamy dowolne stany relaizujące $q=q_A+q_B=6$. Jest to taka sam sytacja jak w zad 3b, dla $N=q$ i $R=4$ czyli $\Omega_{A+B}=84$.
    \item Prawdopodobieństwo $q_A=6$ wynosi $7/84=1/12$ bo $7$ stanów daje $E_A=6$, a wszystkie są równie prawdopodobne.
    \item Najbardziej prawdopodobne jest $q_A=3$ bo liczba stanów jest $4\cdot 4=16$, czyli $p=16/84=1/3$. Dla porównania $p(q_A=2)=p(q_A=4)=5\cdot 3/84=5/28$ oraz $p(q_A=1)=p(q_A=5)=6\cdot 2/84=1/7$ a $p(q_A=6)=p(q_A=0)=7/84=1/12$. 
    \item Entropia $S=k\ln\Omega$, gdzie $k$ -- stała Boltzmanna.
    \begin{align*}
    S_{A\cdot B} =& k_B \ln \Omega_{A \cdot B} = k_B \ln 12 \approx 2,485 k_B\; ,\\
    S_{A + B} =& k_B \ln \Omega_{A + B} = k_B \ln 84 \approx 4,431 k_B\;, \\
    \Delta S =& S_{A + B} - S_{A\cdot B} = k_B \ln \frac{84}{12} = k_B \ln 7 \approx 1,946 k_B \;.
    \end{align*}
\end{enumerate}
...
\]
Dla $N = 30$ i $q = 30$ dostajemy:
\[
\ln \Omega \approx 59 \ln 59 - 30 \ln 30 -29 \ln 29 \approx40.89,
\]
a entropia
\[
S = k_B \ln \Omega \approx 40.89 k_B \approx 40.89 \times 1.3806 \times 10^{-23} \left[\frac{J}{K}\right] \approx 5.6 \times 10^{-22}\left[\frac{J}{K}\right]
\]
\zadanie
...
\resizebox{\linewidth}{!}{\includegraphics{tydzien_11_rys2.png}}
\end{wrapfigure}
\zaddom
...
\end{enumerate}
{\bf \em Odpowiedź:}  a) $\Omega_A=1$, $\Omega_B=6$, ~~ b) $\Omega=19$, ~~ c)   $0$,~~ d) $S=k\log\Omega$.
...
{\bf \em Odpowiedź:} Liczba kombinacji bez powtórzeń $\left( \begin{tabular}{c}  N \\ $n$\end{tabular} \right)^2$.
