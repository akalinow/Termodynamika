...
\begin{wrapfigure}{r}{0.45\linewidth}\vspace{-0.1cm}
  \resizebox{\linewidth}{!}{\includegraphics[width=6cm]{termos.png}}
...
Wprowadźmy następujące oznaczenia: $I_1$: strumień energii emitowanej przez pierwszą płytkę, $I_2$: strumień energii emitowanej przez drugą płytkę, \ldots, $I_{n+2}$: strumień energii emitowanej przez ostatnią płytkę. Poza tym zakładamy, że ustawione płyty są równowadze. To oznacza, że strumień energii pomiędzy każdymi sąsiednimi płytami jest dokładnie taki sam, czyli możemy zapisać, że:

\begin{equation}
I_{1}-I_{2} = I_{2}-I_{3} = \ldots = I_{n+1} - I_{n+2}
\end{equation}

Zapiszmy teraz jak wygląda strumień energii dla jakiejś $k$-tej płyty:

\begin{equation}
I_{k-1} - I_{k} = I_{k} - I_{k+1} \implies 2I_{k} = I_{k-1}+I_{k+1}.
\end{equation}

Ostatnia równość jest spełniona jeżeli $I_{k}$ to elementy ciągu artmetycznego. A zatem możemy zapisać, że:

\begin{equation}
\label{ciag-artmetyczny}
I_{k} = I_{1} + (k-1) \cdot r,
\end{equation}

gdzie stałe $I_1$ oraz $r$ wyznaczymy wykorzystując znajomość temperatur skrajnych płytek. Wiemy, że: $I_1 = \sigma T_{1}^4$ oraz $I_{n+2} = \sigma T_{2}^{4}$, a to oznacza, że:

\begin{equation}
\sigma T_{2}^{4} = \sigma T^{4}_{1} + (n+2-1) \cdot r \implies r = \frac{\sigma (T_{2}^{4} - T_{1}^{4})}{n+1}.
\end{equation}

Zatem wracając do równania \eqref{ciag-artmetyczny} dostajemy, że:

\begin{equation}
I_{k} = \sigma T^{4}_{1} + \frac{\sigma \cdot (k-1)}{n+1}(T_{2}^{4}-T_{1}^{4}).
\end{equation}

Wiedząc, że $I_{k} = \sigma T_{p_{k}}^{4}$ otrzymujemy, że

\begin{equation}
T_{p_{k}}^{4} = T_{1}^{4} + \frac{k-1}{n+1}(T_{2}^{4}-T_{1}^{4}). 
\end{equation}

Możemy teraz policzyć strumień enrgii pomiędzy płytami:

\begin{equation}
I_{1} - I_{2} = -r = \frac{\sigma(T_{1}^{4}-T_{2}^{4})}{n+1}.
\end{equation}

A to oznacza, że w wyniku wstawiniu wstawineiu płytek, strumień energii zmniejszy się $(n+1)$ razy.
